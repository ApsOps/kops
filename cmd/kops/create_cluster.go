...
func NewCmdCreateCluster(f *util.Factory, out io.Writer) *cobra.Command {
	...
	cmd.Flags().StringVar(&options.Networking, "networking", "kubenet", "Networking mode to use.  kubenet (default), classic, external, cni, vxlan.")
...
func RunCreateCluster(f *util.Factory, cmd *cobra.Command, args []string, out io.Writer, c *CreateClusterOptions) error {
	...
	case "vxlan":
		cluster.Spec.Networking.VXLAN = &api.VXLANNetworkingSpec{}
	default:
